...
export function VisitCompletionModal({
  ...
  const installmentData = useOfflineQuery(
    api.installments.getinstallment,
    clerkId && installmentId ? { clerkId, installmentId } : "skip",
    {
      table: "installments",
      // Convex validates IDs before making a request; a local UUID must stay
      // entirely in Dexie until its parent record has synced.
      skipConvex: Boolean(installmentId && isLocalId(String(installmentId))),
      filter: (records) => records.filter((installment) =>
        installment._localId === installmentId || installment._serverId === installmentId
      ),
      select: (records) => records[0] ?? null,
      initialLocalValue: null,
    }
  );
